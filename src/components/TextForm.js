  ...
  return (
    <div className='container-xl mt-5'>
      ...
      <div className={`wrapper my-4 text-${props.mode === 'light'?'dark':'light'}`}>
        ...
        <p className={`fs-6 fst-italic text-${props.mode === 'light'?'success':'info'} lh-sm mb-2`}>{text.split(' ').length} words and {text.length} characters</p>
        <p className={`fs-6 fst-italic text-${props.mode === 'light'?'success':'info'} lh-sm`}>{0.008 * text.split(' ').length} Minutes to read</p>
      </div>
